function [predA,predZ] = dawid_skene_context(settings, x, nQueryUrls, nLabelers, nLabels, K, ...
...
%majorityvote½á¹û
beta_alpha=ones(nLabelers,1)+1+settings.EPS;
beta_beta=ones(nLabelers,1)+settings.EPS;
...
%   disp(lambda);
    cost_function_E(iter) = compute_q_dawid_skene_context(x,lambda,pi_vec,nLabels,alpha,beta_alpha,beta_beta,K,r);
...
cost_function_M(iter) = ...
        compute_q_dawid_skene_context(x,lambda,pi_vec,nLabels,alpha,beta_alpha,beta_beta,K,r);
...
function op = compute_q_dawid_skene_context(x,lambda,pi_vec,nLabels,alpha,beta_alpha,beta_beta,K,r)
