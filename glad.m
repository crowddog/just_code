function [predZ,predA]= glad(settings, x, nQueryUrls, nLabelers, nLabels, K, ...
    v, init_id, difficulty, expt)
% EM for GLAD model
% M-step uses minimize.m for conjugate gradient optimization


alpha_mu = 1;   % gaussian prior over alpha_n
alpha_sigma2 = 1;
log_beta_mu = 1;    % gaussian prior over log(beta_m)
log_beta_sigma2 = 1;
DEBUG = 0;
...
% alpha = sqrt(alpha_sigma2)*(randn(nLabelers,1)/2);
% log_beta = sqrt(log_beta_sigma2)*(randn(nQueryUrls,1)/2);
beta = exp(log_beta);
pi_vec = rand(1,K);
...
for iter = 1:settings.maxIter
    ...
    cost_function_E(iter) = ...
        compute_q_glad(x,lambda,pi_vec,alpha,log_beta,nLabels,K,alpha_mu,alpha_sigma2,log_beta_mu,log_beta_sigma2);
    ...
    % alpha update
    negQ_gradnegQ_alpha = @(alpha) deal(-compute_q_glad(x,lambda,pi_vec,alpha,log_beta,nLabels,K,alpha_mu,alpha_sigma2,log_beta_mu,log_beta_sigma2), ...
        -compute_grad_alpha(x, alpha, beta, alpha_mu, alpha_sigma2, lambda, ii));
    if DEBUG
    ...
    if DEBUG
        if (compute_q_glad(x,lambda,pi_vec,alpha_new,log_beta,nLabels,K,alpha_mu,alpha_sigma2,log_beta_mu,log_beta_sigma2) < ...
                cost_function_E(iter) )
    ...
    % beta update
    negQ_gradnegQ_log_beta = @(log_beta) deal(-compute_q_glad(x,lambda,pi_vec,alpha,log_beta,nLabels,K,alpha_mu,alpha_sigma2,log_beta_mu,log_beta_sigma2), ...
        -compute_grad_log_beta(x, alpha, log_beta, alpha_mu, alpha_sigma2, lambda, ii));
    if DEBUG
    ...
    if DEBUG
        if (compute_q_glad(x,lambda,pi_vec,alpha,...
                log_beta_new,nLabels,K,alpha_mu,alpha_sigma2,log_beta_mu,log_beta_sigma2) > ...
                cost_function_E(iter) )
...
function op = compute_grad_alpha(x, alpha, beta, alpha_mu, alpha_sigma2, lambda, ii)
ss =  sigmoid(alpha(x(:,2)) .* beta(x(:,1)));
tmp = lambda(ii) .* beta(x(:,1)) - ss .* beta(x(:,1));
op = accumarray(x(:,2),tmp) - (alpha-alpha_mu)/(alpha_sigma2);
end

function op = compute_grad_log_beta(x, alpha, log_beta, log_beta_mu, log_beta_sigma2, lambda, ii)
...
function op = compute_q_glad(x,lambda,pi_vec,alpha,log_beta,nLabels,K,alpha_mu,alpha_sigma2,log_beta_mu,log_beta_sigma2)
EPS = 1e-16;
...
op = op - 0.5*( (1/log_beta_sigma2)*sum((log_beta-log_beta_mu).^2) + (1/alpha_sigma2)*sum((alpha-alpha_mu).^2) );%mapµÄÒò×Ó
beta = exp(log_beta);
ss = sigmoid(alpha(x(:,2)).*beta(x(:,1)));
ss = max(ss,EPS);
ss = min(ss,1-EPS);
